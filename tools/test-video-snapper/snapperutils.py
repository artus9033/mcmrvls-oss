...
def listWorkingCameraPorts(logger: Logger, maxScannedPortIndex: int = 10) -> List[int]:
    workingPorts: None | List[int] = None
    ...
    if os.name != "nt":
        ...
        if isCommandAvailable(["which", "v4l2-ctl"]):
            ...
            )  # somehow ffmpeg outputs to stderr...

            if output is None:
                logger.error(
                    "Error checking via command line for cameras on the system"
                )
            ...
            for line in output:
                ...
                for match in matches:
                    index = match
                    logger.info(
                        f"> Hit - detected camera on port {index} with v4l2-ctl:", line
                    )
                    workingPorts.append(int(index))

            return workingPorts

        # fallback if v4l2-utils are not installed
        if isCommandAvailable(["which", "ffmpeg"]) and isCommandAvailable(
            ["which", "awk"]
        ):
            result = subprocess.run(
            ...
            for line in output:
                ...
                for match in matches:
                    ...
                    if "Capture screen" in name or "FaceTime" in name:
                        logger.info(
                            f"Discarding detected camera: Index: {index}, Name: {name}"
                        )
                        continue
        ...
        while portIndex <= maxScannedPortIndex:
            try:
                ...
                if not camera.isOpened():
                    ...
                    if is_reading:
                        logger.info(
                            f"Camera on port {portIndex} works and has resolution ({h} x {w})"
                        )
                        workingPorts.append(portIndex)
                    else:
                        logger.warning(
                            f"Camera on port {portIndex} has resolution {h} x {w} but does not emit data"
                        )
